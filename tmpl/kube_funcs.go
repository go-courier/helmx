...
func ToKubeVolumes(s spec.Spec) kubetypes.KubeVolumes {
	...
	for name, v := range s.Volumes {
		if v.EmptyDir == nil || len(v.EmptyDir.(map[interface{}]interface{})) == 0 {
			v.EmptyDir = struct{}{}
		} else {
			v.EmptyDir = struct {
				...
				SizeLimit: v.EmptyDir.(map[interface{}]interface{})["sizeLimit"].(string),
				Medium:    v.EmptyDir.(map[interface{}]interface{})["medium"].(string),
			}
		}
